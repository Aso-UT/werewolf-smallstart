...
class GeminiLodge(humanConnection: HumanConnection) : Lodge(humanConnection) {
    override fun assignments(): List<Pair<Player, Role>> {
        ...
        val aiPlayers = roles.drop(1).mapIndexed { i, role ->
            AiPlayer(role, AI_NAMES[i], GeminiLanguageModel(), Instruction(AI_NAMES[i], personalities[i])) to role
        }

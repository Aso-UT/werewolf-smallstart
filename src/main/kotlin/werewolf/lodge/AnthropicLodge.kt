...
class AnthropicLodge(humanConnection: HumanConnection) : Lodge(humanConnection) {
    override fun assignments(): List<Pair<Player, Role>> {
        ...
        val aiPlayers = roles.drop(1).mapIndexed { i, role ->
            AiPlayer(role, AI_NAMES[i], AnthropicLanguageModel(HAIKU_MODEL), Instruction(AI_NAMES[i], personalities[i])) to role
        }
